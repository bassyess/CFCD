...
_C.MODEL.LOSSES.LAMDA = 1.0
_C.MODEL.LOSSES.MARGIN = 0.1
...
# Margin and Scale for Arcface head
_C.MODEL.HEADS.MARGIN = 0.15
_C.MODEL.HEADS.SCALE = 128
# Alpha for MadaCos head
_C.MODEL.HEADS.ALPHA = 0.1
# Head: Arcface or MadaCos
_C.MODEL.HEADS.HEAD = "Arcface"
...
# batch size in single gpu must be divisible by the tuple_size
_C.DATA_LOADER.TUPLE_SIZE = 8

# dataset : DataSet
_C.DATA_LOADER.DATASET = "DataSet"

# date sampler
_C.DATA_LOADER.SAMPLER = "DistributedSampler"
# ------------------------------------------------------------------------------------ #
# Memory options
# ------------------------------------------------------------------------------------ #
_C.MEM = CfgNode()
...
def load_cfg_from_file(cfg_file="config.yaml"):
    # """Loads config from specified output directory."""
    # cfg_file = os.path.join(out_dir, cfg_dest)
    _C.merge_from_file(cfg_file)
