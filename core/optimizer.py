...
def construct_optimizer(model):
    ...
    if cfg.BN.USE_CUSTOM_WEIGHT_DECAY:
        ...
        p_bn = [p for n, p in model.named_parameters() if "bn" in n and p.requires_grad]
        p_non_bn = [p for n, p in model.named_parameters() if "bn" not in n and p.requires_grad]
        optim_params = [
            {"params": p_bn, "weight_decay": cfg.BN.CUSTOM_WEIGHT_DECAY},
            {"params": p_non_bn, "weight_decay": cfg.OPTIM.WEIGHT_DECAY},
        ]
        print("train parameters", optim_params)
    else:
        train_parameters = [n for n, p in model.named_parameters() if p.requires_grad]
        print("train parameters", train_parameters)
